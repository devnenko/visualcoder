...
export class VisRect {
    ...
    drawHierarchy(parentSize) {
        this.drawSingle(parentSize);
        for (const child of this.children) {
            if (instanceOfIVisibleObject(child)) {
                child.drawHierarchy(this.absEdges);
            }
            //child.drawInHierarchy(parentSize); should find next instance of IVisibleObject
        }
    }
    resize(parentSize) {
