import { FunctionNames2, PinType } from "../../code/compiler.js";
import { colorCreator } from "../../ui/color.js";
import { MouseHandler } from "../../ui/event_handlers/mouse.js";
import { Rect } from "../../ui/rect.js";
import { boundingShape } from "../../ui/shape.js";
import { Text } from "../../ui/text.js";
import { EConstraintsY } from "../../ui/types/constraints.js";
import { EMouseType } from "../../ui/types/mouse.js";
import { VerticalBox } from "../../ui/vertical_box.js";
import { ViewContentArea } from "./view.js";
import { DefaultHoverPressButton } from "../button.js";
...
export class Pin extends Rect {
    constructor(parent, canvas, isInPin, pinVal) {
        ...
        this.pinVal = pinVal;
        if (isInPin == true) {
            this.fixedPos.y = 35 + 25 * parent.inPins.length;
            parent.inPins.push(this);
        }
        else {
            this.fixedPos.x = parent.fixedSize.w - this.fixedSize.w;
            this.fixedPos.y = 35 + 25 * parent.outPins.length;
    ...
    onMouseUp(type, pos, topMost) {
        ...
        this.prevLine?.destroyHierarchy();
        this.prevLine = null;
        boundingShape.drawHierarchy;
    }
    onKeyPress(key) {
...
export class Block extends Rect {
    ...
    onMouseMoveDown(type, pos, topMost) {
        if (topMost == true) {
            ...
            this.fixedPos.x = relPos.x - this.mouseOffsetBlock.x;
            this.fixedPos.y = relPos.y - this.mouseOffsetBlock.y;
        }
    }
}
export class BlockSelector extends VerticalBox {
    ...
    addButton() {
        for (const fn of FunctionNames2) {
            const button = new DefaultHoverPressButton(this, this.canvas);
            ...
            button.onCLick = (type, pos) => {
                ...
                block.fixedPos = MouseHandler.getRelativeMousePos(this.parent, pos);
                ...
                if (fn.args?.in != undefined) {
                    for (const inArgs of fn.args.in) {
                        ...
                        switch (inArgs.pinType) {
                            ...
                            case PinType.string:
                                pin.color = "green";
                                pin.text = new Text(pin, pin.canvas);
                                pin.text.color = "red";
                ...
                if (fn.args?.out != undefined) {
                    for (const outArgs of fn.args.out) {
                        ...
                        switch (outArgs.pinType) {
                            ...
                            case PinType.string:
                                pin.color = "green";
                                pin.text = new Text(this, this.canvas);
                                pin.text.color = "red";
    ...
    onMouseDown(type, pos, topMost) {
        if (topMost == true && (type == EMouseType.left || type == EMouseType.touch)) {
            if (this.selector == null) {
                ...
                this.selector.fixedPos = MouseHandler.getRelativeMousePos(this, pos);
            }
            this.selector.fixedPos = MouseHandler.getRelativeMousePos(this, pos);
            ...
            if (this.selector != null) {
                this.selector.destroyHierarchy();
                this.selector = null;
            }
        }
    }
