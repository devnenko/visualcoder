...
export class ImageObj{
    static defaultImage=new ImageObj("pause.svg");
    htmlImage;

    constructor(src:string){
        this.htmlImage = new Image();
        this.htmlImage.src = "/icons/".concat(src);
    }
    setColor(color:string){

    }
}
export interface SvgRectConfig extends IRectConfig{
    image?:ImageObj
}

export class ImageRect<Config = SvgRectConfig> extends Rect<Config> {
    image:ImageObj;
    constructor(config?:Config) {
        super();
        this.image=ImageObj.defaultImage;
        if(this.image.htmlImage.complete)//already loaded
        {
            boundingRect.draw();
        }else{
            this.image.htmlImage.onload =function() {
                boundingRect.draw();
            }
        }
    ...
    setImage(image:ImageObj){
        if(!(image.htmlImage.complete))//not already loaded
        {
            image.htmlImage.onload =function() {
                boundingRect.draw();
            }
        }
    }
